...
function Carousel(sel, h, w, options) {
	...
	function initialize(sel, h, w, options) {
		...
		self.$slider.width( self.$images.length * self.$imageWidth);
		...
		self.imgIndex = 0; // initialize becasuse we know this is zero
		self.lastImgIndex = self.$images.length - 1; // highest index we can get


	}

	// slide the slider left by one image width
	self.moveToLeft = function(imageIndex) {
	...
	self.moveToRight = function(imageIndex) {
		if (self.imgIndex > 0) {
			self.animateTransition(+1)
		}
	}
	...
	self.animateTransition = function(direction) {
		...
		var nextLeft = currLeft + self.imageWidth * direction; // direction is either plus 1 or -1
		self.$slider.animate(
			{left: nextLeft},
